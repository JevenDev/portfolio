      ...
      <div class="frame-block">
        <div class="grid gap-7 border-b border-line px-5 pb-10 pt-14 md:grid-cols-[1fr_auto] md:gap-8 md:px-8 md:pb-12 md:pt-16">
          <div class="space-y-4">
            ...
            <p class="max-w-[32rem] text-sm leading-relaxed text-muted md:text-base" data-reveal>
          ...
          <div class="flex flex-wrap items-start gap-3 md:justify-end" data-reveal>
            <button
              type="button"
              class="focus-ring border border-ink bg-ink px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-paper transition hover:bg-paper hover:text-ink"
            ...
            <button
              type="button"
              class="focus-ring border border-line px-5 py-2.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-ink transition hover:border-ink/40"
          ...
          <h1
            ref="titleRef"
            class="md:col-span-2 max-w-[19ch] font-display text-[clamp(3rem,11vw,11.5rem)] font-semibold uppercase leading-[0.93] tracking-[-0.05em] text-ink"
          >
            {{ name }}
          </h1>
        ...
        <div class="space-y-4 p-3 md:p-4">
          ...
          >
            <figure class="overflow-hidden border border-line bg-shell">
              <img
          ...
          <div class="grid gap-4 px-2 pb-2 md:grid-cols-[1fr_auto] md:items-end md:gap-8 md:px-3">
            ...
            <div class="space-y-2 md:text-right" data-reveal>
              <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted">{{ location }}</p>
              <p v-if="heroProject?.year" class="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted">
                {{ heroProject.year }}
              </p>
              <button
                v-if="heroProject"
                type="button"
                class="focus-ring mt-2 border border-line px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-ink transition hover:border-ink/40"
                @click="emit('open-project', heroProject)"
              >
                Open Project
              </button>
